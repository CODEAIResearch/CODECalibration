...
def get_threshold(scores,labels, predictions):
...
def main():
    ...
    for method in uncertainty_methods:
        ...
        if method == "vanilla":
            ...
            """isotonic regression"""
            isotonic_scaled = isotonic_regression(1-eval_vanilla, eval_correctness, 1-test_vanilla, "ece_isotonic",method_dir)
            ...
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_isotonic_scaled = isotonic_regression(eval_vanilla, 1-eval_correctness, test_vanilla, "uce_isotonic",method_dir)
            ...
            """isotonic regression"""
            isotonic_scaled = isotonic_regression(1-eval_margin, eval_correctness, 1-test_margin, "ece_isotonic",method_dir)
            ECE, ACC, MAX_ECE,brier_score = draw_reliability_graph((isotonic_scaled).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_isotonic_scaled = isotonic_regression(eval_margin, 1-eval_correctness, test_margin, "uce_isotonic",method_dir)
            UCE,MAX_UCE  = draw_uce_reliability_graph(uncertain_isotonic_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_bin = get_threshold(uncertain_isotonic_scaled, test_label,test_preds)
            scores_path = os.path.join(method_dir, "isotonic_scores.txt")
            ...
            """isotonic regression"""
            isotonic_scaled = isotonic_regression(1-eval_ratio, eval_correctness, 1-test_ratio, "ece_isotonic",method_dir)
            ECE, ACC, MAX_ECE,brier_score = draw_reliability_graph((isotonic_scaled).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_isotonic_scaled = isotonic_regression(eval_ratio, 1-eval_correctness, test_ratio, "uce_isotonic",method_dir)
            UCE,MAX_UCE  = draw_uce_reliability_graph(uncertain_isotonic_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_bin = get_threshold(uncertain_isotonic_scaled, test_label,test_preds)
            scores_path = os.path.join(method_dir, "isotonic_scores.txt")
            ...
            """isotonic regression"""
            isotonic_scaled = isotonic_regression(1-eval_best, eval_correctness, 1-test_best, "ece_isotonic",method_dir)
            ECE, ACC, MAX_ECE,brier_score = draw_reliability_graph((isotonic_scaled).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_isotonic_scaled = isotonic_regression(eval_best, 1-eval_correctness, test_best, "uce_isotonic",method_dir)
            UCE,MAX_UCE  = draw_uce_reliability_graph(uncertain_isotonic_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_bin = get_threshold(uncertain_isotonic_scaled, test_label,test_preds)
            scores_path = os.path.join(method_dir, "isotonic_scores.txt")
            ...
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_isotonic_scaled = isotonic_regression(eval_sws, 1-eval_correctness, test_sws, "swsuce_isotonic",method_dir)
