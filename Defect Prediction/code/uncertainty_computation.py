...
def compute_sws(logits):
    ...
    max_probs, _ = logits.max(dim=-1)  # Take the highest probability per sample
...
def compute_pv(logits):
    ...
    """
    #mc_probs = F.softmax(logits, dim=-1)  # Convert logits to probabilities
    pv_scores = logits.var(dim=0).mean(dim=1)  # Variance over MC samples, mean over classes
...
def get_threshold_old(
    ...
    min_frac=0.20, # minimum fraction of samples the selected group must cover
    n_thresholds=200,
):
    """
...
def main():
    ...
    print(eval_correctness, eval_probs.shape)
    ...
    uncertainty_methods = ["vanilla", "entropy","mutual", "margin", "least", "ratio", "best", "mcd","ensemble" ] #["mcd","ensemble", "dsmg"]
    #uncertainty_methods = ["ensemble"]
    for method in uncertainty_methods:
        ...
        if method == "vanilla":
            ...
            os.makedirs(method_dir, exist_ok=True)
            ECE, ACC, MAX_ECE,brier_score = draw_reliability_graph((test_vanilla).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "before_scale" )
            UCE, MAX_UCE = draw_uce_reliability_graph(1-test_vanilla.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "before_scale" )
            uncertain_bin = get_threshold(test_vanilla, test_label,test_preds)
            ...
            with open(scores_path, "w") as f:
                ...
                f.write(f"Distribution of uncertainty\n\n: {uncertain_bin}")

            record_result(method, "before_scale", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)

            #apply temperature scaling

            uce_scaled_values = train_UCE_temperature(args, model, 1-eval_vanilla, eval_correctness, 1-test_vanilla, test_correctness)
            uce_scaled_values = torch.tensor(uce_scaled_values)
            ece_scaled_values = train_ECE_temperature(args, model, eval_vanilla, eval_correctness, test_vanilla, test_correctness)
            ...
            uncertain_bin = get_threshold(uce_scaled_values, test_label,test_preds)
            ...
            record_result(method, "after_scale", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)

            ####platt scaling
            plat_scaled = platt_scaling_pipeline(eval_vanilla, eval_correctness, test_vanilla, "vanilla",method_dir)
            ...
            uncertain_plat_scaled = platt_scaling_pipeline(1-eval_vanilla, 1-eval_correctness, 1-test_vanilla, "new_uncertain_vanilla",method_dir)
            ...
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_bin = get_threshold(uncertain_plat_scaled, test_label,test_preds)
            ...
            record_result(method, "platt_sclae", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)


            """isotonic regression"""
            isotonic_scaled = isotonic_regression(eval_vanilla, eval_correctness, test_vanilla, "ece_isotonic",method_dir)
            ECE, ACC, MAX_ECE,brier_score = draw_reliability_graph((isotonic_scaled).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_isotonic_scaled = isotonic_regression(1-eval_vanilla, 1-eval_correctness, 1-test_vanilla, "uce_isotonic",method_dir)
            UCE,MAX_UCE  = draw_uce_reliability_graph(uncertain_isotonic_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_bin = get_threshold(uncertain_isotonic_scaled, test_label,test_preds)
            scores_path = os.path.join(method_dir, "isotonic_scores.txt")
            with open(scores_path, "w") as f:
                f.write(f" ECE: {ECE:.4f}\n")
                f.write(f" UCE: {UCE:.4f}\n")
                f.write(f" Brier Score: {brier_score:.4f}\n")
            ...
            record_result(method, "isotonic", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)

            write_excel_per_method(method, method_dir, one_sheet=True) 
            ...
            uncertain_bin = get_threshold(test_entropy, test_label,test_preds)
            scores_path = os.path.join(method_dir, "before_scores.txt")
            with open(scores_path, "w") as f:
                f.write(f" ECE: {ECE:.4f}\n")
                f.write(f" UCE: {UCE:.4f}\n")
            ...
            record_result(method, "before", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)
            ...
            record_result(method, "after", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)
            
            ####platt scaling
            plat_scaled = platt_scaling_pipeline(1-eval_entropy, eval_correctness, 1-test_entropy, "myentropy",method_dir)
            ECE, ACC, MAX_ECE,brier_score = draw_reliability_graph((plat_scaled).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_plat_scaled = platt_scaling_pipeline(eval_entropy, 1-eval_correctness, test_entropy, "mynewuncertain_entropy",method_dir)
            UCE,MAX_UCE  = draw_uce_reliability_graph(uncertain_plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_bin = get_threshold(uncertain_plat_scaled, test_label,test_preds)
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            ...
            record_result(method, "platt", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)
            ...
            """isotonic regression"""
            isotonic_scaled = isotonic_regression(1-eval_entropy, eval_correctness, 1-test_entropy, "ece_isotonic",method_dir)
            ECE, ACC, MAX_ECE,brier_score = draw_reliability_graph((isotonic_scaled).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_isotonic_scaled = isotonic_regression(eval_entropy, 1-eval_correctness, test_entropy, "uce_isotonic",method_dir)
            UCE,MAX_UCE  = draw_uce_reliability_graph(uncertain_isotonic_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            uncertain_bin = get_threshold(uncertain_isotonic_scaled, test_label,test_preds)
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            #uncertain_bin = get_threshold(uncertain_isotonic_scaled, test_label,test_preds)
            ...
            uce_scaled_values1 = uncertain_isotonic_scaled.cpu().numpy()
            ...
            uncertain_bin = get_threshold(test_mutual, test_label,test_preds)
            scores_path = os.path.join(method_dir, "before_scores.txt")
            with open(scores_path, "w") as f:
                f.write(f" ECE: {ECE:.4f}\n")
                f.write(f" UCE: {UCE:.4f}\n")
            ...
            record_result(method, "plat", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)

            """isotonic regression"""
            isotonic_scaled = isotonic_regression(1-eval_mutual, eval_correctness, 1-test_mutual, "ece_isotonic",method_dir)
            ECE, ACC, MAX_ECE,brier_score = draw_reliability_graph((isotonic_scaled).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_isotonic_scaled = isotonic_regression(eval_mutual, 1-eval_correctness, test_mutual, "uce_isotonic",method_dir)
            UCE,MAX_UCE  = draw_uce_reliability_graph(uncertain_isotonic_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_bin = get_threshold(uncertain_isotonic_scaled, test_label,test_preds)
            scores_path = os.path.join(method_dir, "isotonic_scores.txt")
            ...
            method_dir = os.path.join(ROOT_DIR, method)
            os.makedirs(method_dir, exist_ok=True)
            ECE, ACC,MAX_ECE, brier_score = draw_reliability_graph((test_margin).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "before_scale" )
            UCE,MAX_UCE  = draw_uce_reliability_graph(1-test_margin.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "before_scale" )
            uncertain_bin = get_threshold(test_margin, test_label,test_preds)
            scores_path = os.path.join(method_dir, "before_scores.txt")
            with open(scores_path, "w") as f:
                f.write(f" ECE: {ECE:.4f}\n")
                f.write(f" UCE: {UCE:.4f}\n")
            ...
            uce_scaled_values = train_UCE_temperature(args, model, 1-eval_margin, eval_correctness, 1-test_margin, test_correctness)
            uce_scaled_values = torch.tensor(uce_scaled_values)
            ece_scaled_values = train_ECE_temperature(args, model, eval_margin, eval_correctness, test_margin, test_correctness)
            ece_scaled_values = torch.tensor(ece_scaled_values)
            ECE, ACC,MAX_ECE, brier_score = draw_reliability_graph((ece_scaled_values).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "after_scale" )
            UCE,MAX_UCE  = draw_uce_reliability_graph(uce_scaled_values.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "after_scale" )
            uncertain_bin = get_threshold(uce_scaled_values, test_label,test_preds)
            ...
            record_result(method, "after", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)
            ####platt scaling
            plat_scaled = platt_scaling_pipeline(eval_margin, eval_correctness, test_margin, "margin",method_dir)
            ECE, ACC, MAX_ECE, brier_score = draw_reliability_graph((plat_scaled).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_plat_scaled = platt_scaling_pipeline(1-eval_margin, 1-eval_correctness, 1-test_margin, "newuncertain_margin",method_dir)
            UCE,MAX_UCE  = draw_uce_reliability_graph(uncertain_plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_bin = get_threshold(uncertain_plat_scaled, test_label,test_preds)
            scores_path = os.path.join(method_dir, "platscaled_scores.txt")
            ...
            """isotonic regression"""
            isotonic_scaled = isotonic_regression(eval_margin, eval_correctness, test_margin, "ece_isotonic",method_dir)
            ECE, ACC, MAX_ECE,brier_score = draw_reliability_graph((isotonic_scaled).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_isotonic_scaled = isotonic_regression(1-eval_margin, 1-eval_correctness, 1-test_margin, "uce_isotonic",method_dir)
            UCE,MAX_UCE  = draw_uce_reliability_graph(uncertain_isotonic_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_bin = get_threshold(uncertain_isotonic_scaled, test_label,test_preds)
            scores_path = os.path.join(method_dir, "isotonic_scores.txt")
            ...
            uncertain_bin = get_threshold(test_least, test_label,test_preds)
            scores_path = os.path.join(method_dir, "before_scores.txt")
            with open(scores_path, "w") as f:
                f.write(f" ECE: {ECE:.4f}\n")
                f.write(f" UCE: {UCE:.4f}\n")
            ...
            """isotonic regression"""
            isotonic_scaled = isotonic_regression(1-eval_least, eval_correctness, 1-test_least, "ece_isotonic",method_dir)
            ECE, ACC, MAX_ECE,brier_score = draw_reliability_graph((isotonic_scaled).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_isotonic_scaled = isotonic_regression(eval_least, 1-eval_correctness, test_least, "uce_isotonic",method_dir)
            UCE,MAX_UCE  = draw_uce_reliability_graph(uncertain_isotonic_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_bin = get_threshold(uncertain_isotonic_scaled, test_label,test_preds)
            scores_path = os.path.join(method_dir, "isotonic_scores.txt")
            ...
            method_dir = os.path.join(ROOT_DIR, method)
            os.makedirs(method_dir, exist_ok=True)
            ECE, ACC,MAX_ECE, brier_score = draw_reliability_graph((test_ratio).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "before_scale" )
            UCE,MAX_UCE  = draw_uce_reliability_graph(1-test_ratio.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "before_scale" )
            uncertain_bin = get_threshold(test_ratio, test_label,test_preds)
            scores_path = os.path.join(method_dir, "before_scores.txt")
            with open(scores_path, "w") as f:
                f.write(f" ECE: {ECE:.4f}\n")
                f.write(f" UCE: {UCE:.4f}\n")
            ...
            uce_scaled_values = train_UCE_temperature(args, model, 1-eval_ratio, eval_correctness, 1-test_ratio, test_correctness)
            uce_scaled_values = torch.tensor(uce_scaled_values)
            ece_scaled_values = train_ECE_temperature(args, model, eval_ratio, eval_correctness, test_ratio, test_correctness)
            ece_scaled_values = torch.tensor(ece_scaled_values)
            ECE, ACC, MAX_ECE,brier_score = draw_reliability_graph((ece_scaled_values).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "after_scale" )
            UCE,MAX_UCE  = draw_uce_reliability_graph(uce_scaled_values.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "after_scale" )
            uncertain_bin = get_threshold(uce_scaled_values, test_label,test_preds)
            ...
            ####platt scaling
            plat_scaled = platt_scaling_pipeline(eval_ratio, eval_correctness, test_ratio, "ratio",method_dir)
            ECE, ACC,MAX_ECE, brier_score = draw_reliability_graph((plat_scaled).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_plat_scaled = platt_scaling_pipeline(1-eval_ratio, 1-eval_correctness, 1-test_ratio, "newuncertain_ratio",method_dir)
            UCE,MAX_UCE  = draw_uce_reliability_graph(uncertain_plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_bin = get_threshold(uncertain_plat_scaled, test_label,test_preds)
            scores_path = os.path.join(method_dir, "platscaled_scores.txt")
            ...
            """isotonic regression"""
            isotonic_scaled = isotonic_regression(eval_ratio, eval_correctness, test_ratio, "ece_isotonic",method_dir)
            ECE, ACC, MAX_ECE,brier_score = draw_reliability_graph((isotonic_scaled).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_isotonic_scaled = isotonic_regression(1-eval_ratio, 1-eval_correctness, 1-test_ratio, "uce_isotonic",method_dir)
            UCE,MAX_UCE  = draw_uce_reliability_graph(uncertain_isotonic_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_bin = get_threshold(uncertain_isotonic_scaled, test_label,test_preds)
            scores_path = os.path.join(method_dir, "isotonic_scores.txt")
            ...
            method_dir = os.path.join(ROOT_DIR, method)
            os.makedirs(method_dir, exist_ok=True)
            ECE, ACC,MAX_ECE, brier_score = draw_reliability_graph(test_best.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "before_scale" )
            UCE,MAX_UCE  = draw_uce_reliability_graph((1-test_best.to(device)),test_label.to(device), test_preds.to(device), 10, method_dir, "before_scale" )
            uncertain_bin = get_threshold(test_best, test_label,test_preds)
            scores_path = os.path.join(method_dir, "before_scores.txt")
            with open(scores_path, "w") as f:
                f.write(f" ECE: {ECE:.4f}\n")
                f.write(f" UCE: {UCE:.4f}\n")
            ...
            uce_scaled_values = train_UCE_temperature(args, model, 1-eval_best, eval_correctness, 1-test_best, test_correctness)
            uce_scaled_values = torch.tensor(uce_scaled_values)
            ece_scaled_values = train_ECE_temperature(args, model, eval_best, eval_correctness, test_best, test_correctness)
            ece_scaled_values = torch.tensor(ece_scaled_values)
            ECE, ACC,MAX_ECE, brier_score = draw_reliability_graph((ece_scaled_values).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "after_scale" )
            UCE,MAX_UCE  = draw_uce_reliability_graph(uce_scaled_values.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "after_scale" )
            uncertain_bin = get_threshold(uce_scaled_values, test_label,test_preds)
            ...
            ####platt scaling
            plat_scaled = platt_scaling_pipeline(eval_best, eval_correctness, test_best, "best",method_dir)
            ECE, ACC,MAX_ECE, brier_score = draw_reliability_graph((plat_scaled).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_plat_scaled = platt_scaling_pipeline(1-eval_best, 1-eval_correctness, 1-test_best, "newuncertain_best",method_dir)
            UCE,MAX_UCE  = draw_uce_reliability_graph(uncertain_plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_bin = get_threshold(uncertain_plat_scaled, test_label,test_preds)
            scores_path = os.path.join(method_dir, "platscaled_scores.txt")
            ...
            """isotonic regression"""
            isotonic_scaled = isotonic_regression(eval_best, eval_correctness, test_best, "ece_isotonic",method_dir)
            ECE, ACC, MAX_ECE,brier_score = draw_reliability_graph((isotonic_scaled).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_isotonic_scaled = isotonic_regression(1-eval_best, 1-eval_correctness, 1-test_best, "uce_isotonic",method_dir)
            UCE,MAX_UCE  = draw_uce_reliability_graph(uncertain_isotonic_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_bin = get_threshold(uncertain_isotonic_scaled, test_label,test_preds)
            scores_path = os.path.join(method_dir, "isotonic_scores.txt")
            ...
            if test_mc_logits is None:
                test_mc_logits = monte_carlo_dropout_logits(model, test_dataloader, num_samples=5)
            ...
            UCE,MAX_UCE  = draw_uce_reliability_graph(1-test_sws.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "sws_before_scale" )
            uncertain_bin = get_threshold(test_sws, test_label,test_preds)
            ...
            record_result(method, "before sws", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)
            #apply temperature scaling

            uce_scaled_values = train_UCE_temperature(args, model, 1-eval_sws, eval_correctness, 1-test_sws, test_correctness)
            uce_scaled_values = torch.tensor(uce_scaled_values)
            ...
            record_result(method, "after sws", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)
            ...
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_plat_scaled = platt_scaling_pipeline(1-eval_sws, 1-eval_correctness, 1-test_sws, "newuncertain_sws",method_dir)
            ...
            record_result(method, "plat sws", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)

            isotonic_scaled = isotonic_regression(eval_sws, eval_correctness, test_sws, "swsece_isotonic",method_dir)
            ECE, ACC, MAX_ECE,brier_score = draw_reliability_graph((isotonic_scaled).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "swsisotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_isotonic_scaled = isotonic_regression(1-eval_sws, 1-eval_correctness, 1-test_sws, "swsuce_isotonic",method_dir)
            UCE,MAX_UCE  = draw_uce_reliability_graph(uncertain_isotonic_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "swsisotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_bin = get_threshold(uncertain_isotonic_scaled, test_label,test_preds)
            scores_path = os.path.join(method_dir, "swsisotonic_scores.txt")
            with open(scores_path, "w") as f:
                f.write(f" ECE: {ECE:.4f}\n")
                f.write(f" UCE: {UCE:.4f}\n")
                f.write(f" Brier Score: {brier_score:.4f}\n")
            ...
            record_result(method, "isotonic sws", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)
            ...
            uncertain_bin = get_threshold(test_pv, test_label,test_preds)
            ...
            with open(scores_path, "w") as f:
                ...
                f.write(f"MAX_UCE:{MAX_UCE:.4f}\n ")
                f.write(f"Distribution of uncertainty\n\n: {uncertain_bin}")
            record_result(method, "before pv", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)
            ...
            with open(scores_path, "w") as f:
                ...
                f.write(f"MAX_UCE:{MAX_UCE:.4f}\n ")
                f.write(f"Distribution of uncertainty\n\n: {uncertain_bin}")
            record_result(method, "after pv", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)
            ...
            with open(scores_path, "w") as f:
                ...
                f.write(f"MAX_UCE:{MAX_UCE:.4f}\n ")
                f.write(f"Distribution of uncertainty\n\n: {uncertain_bin}")
            record_result(method, "plat pv", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)

            isotonic_scaled = isotonic_regression(1-eval_pv, eval_correctness, 1-test_pv, "pvece_isotonic",method_dir)
            ECE, ACC, MAX_ECE,brier_score = draw_reliability_graph((isotonic_scaled).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "pvisotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_isotonic_scaled = isotonic_regression(eval_pv, 1-eval_correctness, test_pv, "pvuce_isotonic",method_dir)
            UCE,MAX_UCE  = draw_uce_reliability_graph(uncertain_isotonic_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "pvisotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_bin = get_threshold(uncertain_isotonic_scaled, test_label,test_preds)
            scores_path = os.path.join(method_dir, "pvisotonic_scores.txt")
            with open(scores_path, "w") as f:
                f.write(f" ECE: {ECE:.4f}\n")
                f.write(f" UCE: {UCE:.4f}\n")
                f.write(f" Brier Score: {brier_score:.4f}\n")
                ...
                f.write(f"MAX_UCE:{MAX_UCE:.4f}\n ")
                f.write(f"Distribution of uncertainty\n\n: {uncertain_bin}")
            record_result(method, "isotonic pv", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)
            ...
            uncertain_bin = get_threshold(test_bald, test_label,test_preds)
            ...
            with open(scores_path, "w") as f:
                ...
                f.write(f"MAX_UCE:{MAX_UCE:.4f}\n ")
                f.write(f"Distribution of uncertainty\n\n: {uncertain_bin}")
            record_result(method, "before bald", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)
            ...
            with open(scores_path, "w") as f:
                ...
                f.write(f"MAX_UCE:{MAX_UCE:.4f}\n ")
                f.write(f"Distribution of uncertainty\n\n: {uncertain_bin}")
            record_result(method, "after bald", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)
            ...
            with open(scores_path, "w") as f:
                ...
                f.write(f"MAX_UCE:{MAX_UCE:.4f}\n ")
                f.write(f"Distribution of uncertainty\n\n: {uncertain_bin}")
            record_result(method, "plat bald", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)

            isotonic_scaled = isotonic_regression(1-eval_bald, eval_correctness, 1-test_bald, "baldece_isotonic",method_dir)
            ECE, ACC, MAX_ECE,brier_score = draw_reliability_graph((isotonic_scaled).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "baldisotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_isotonic_scaled = isotonic_regression(eval_bald, 1-eval_correctness, test_bald, "balduce_isotonic",method_dir)
            UCE,MAX_UCE  = draw_uce_reliability_graph(uncertain_isotonic_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "baldisotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_bin = get_threshold(uncertain_isotonic_scaled, test_label,test_preds)
            scores_path = os.path.join(method_dir, "baldisotonic_scores.txt")
            with open(scores_path, "w") as f:
                f.write(f" ECE: {ECE:.4f}\n")
                f.write(f" UCE: {UCE:.4f}\n")
                f.write(f" Brier Score: {brier_score:.4f}\n")
                ...
                f.write(f"MAX_UCE:{MAX_UCE:.4f}\n ")
                f.write(f"Distribution of uncertainty\n\n: {uncertain_bin}")
            record_result(method, "isotonic bald", ECE, ACC, MAX_ECE, brier_score, UCE, MAX_UCE, uncertain_bin)
            write_excel_per_method(method, method_dir, one_sheet=True)
            ...
            uncertain_bin = get_threshold(test_score_normalized, test_label,test_preds)
            scores_path = os.path.join(method_dir, "before_scores.txt")
            with open(scores_path, "w") as f:
                f.write(f" ECE: {ECE:.4f}\n")
                f.write(f" UCE: {UCE:.4f}\n")
            ...
            isotonic_scaled = isotonic_regression(eval_score_normalized, eval_correctness, test_score_normalized, "ece_isotonic",method_dir)
            ECE, ACC, MAX_ECE,brier_score = draw_reliability_graph((isotonic_scaled).to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_isotonic_scaled = isotonic_regression(eval_score_normalized, 1-eval_correctness, test_score_normalized, "uce_isotonic",method_dir)
            UCE,MAX_UCE  = draw_uce_reliability_graph(uncertain_isotonic_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "isotonic" )
            #UCE  = draw_uce_reliability_graph(plat_scaled.to(device),test_label.to(device), test_preds.to(device), 10, method_dir, "plat_scaled" )
            uncertain_bin = get_threshold(uncertain_isotonic_scaled, test_label,test_preds)
            scores_path = os.path.join(method_dir, "isotonic_scores.txt")
