...
def main():
    ...
    if args.model_type in ['codellama', 'starcoder3b', "deepseek","incoder1b",'qwen7b']:
        model = DecoderClassifierLTA(model,config,tokenizer,args, num_classes=5) #DecoderClassifier(model,config,tokenizer,args)
    else:
    ...
    if args.do_test_prob and args.local_rank in [-1, 0]:
        ...
        test_prob_selective_SR(args, model, tokenizer, coverages=[round(x,2) for x in np.arange(0.1, 1.01, 0.1)], exclude_last_class=True)
    return results
