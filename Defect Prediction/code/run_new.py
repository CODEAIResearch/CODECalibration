...
def convert_examples_to_features(js,tokenizer,args):
    ...
    elif args.model_type in ["starcoder3b", "deepseek", 'qwen7b']:
        code_tokens=tokenizer.tokenize(code)
        source_tokens = code_tokens[:args.block_size]
    else:
        code_tokens=tokenizer.tokenize(code)
...
def train(args, accelerator, train_dataset, eval_dataset, model, tokenizer):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        ...
        if args.local_rank in [-1, 0]:
            ...
            if idx == 9:
                checkpoint_prefix = f'checkpoint-acsac/{args.project}'
                output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))                        
                if not os.path.exists(output_dir):
                    os.makedirs(output_dir, exist_ok=True)                        
                accelerator.save_state(output_dir)
            ...
            if results['eval_acc']>best_acc:
                ...
                checkpoint_prefix = f'checkpoint-best-acc/{args.project}'
                output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))                        
                if not os.path.exists(output_dir):
                    os.makedirs(output_dir, exist_ok=True)                        
                accelerator.save_state(output_dir)
...
def main():
    ...
    if args.model_name_or_path:
        if args.model_type in ["starcoder3b","deepseek"]:
    ...
    if args.model_type in ['codellama', 'starcoder3b',"deepseek",'qwen7b']:
    ...
    if args.model_type in ['starcoder3b']:
