...
from transformers import (WEIGHTS_NAME, AdamW, get_linear_schedule_with_warmup,
                          ...
                          DistilBertConfig, DistilBertForMaskedLM, DistilBertForSequenceClassification, DistilBertTokenizer,T5Config, T5ForConditionalGeneration, T5Tokenizer)
...
def fit(net, x, y, optim, loss, device):
    net.train()  # Ensure Dropout & BatchNorm layers (if any) are active
    x = x.to(device)  # Move input tensor to GPU/CPU
    y = y.to(device, dtype=torch.long).reshape(-1)  # Ensure correct dtype & shape

    pred = net(x)  # Forward pass (expects [batch_size, 768] input)

    loss_val = loss(pred, y)  # Compute loss (CrossEntropyLoss expects [batch_size, 2] logits & [batch_size] targets)

    optim.zero_grad()
    loss_val.backward()
    optim.step()

    return loss_val.item()  # Convert to scalar for logging
...
class ImportanceScore(nn.Module):
    ...
    def cal_newdissector(self,pred_vec, original_pred):
        original_pred = original_pred.squeeze(-1)
        pred_vec = F.softmax(pred_vec)
        
        pred_order = torch.argsort(pred_vec, dim=1)

        #print("Shape of pred_vec:", pred_vec.shape)  # Should be [batch_size, num_classes]
        #print("Shape of original_pred (y):", original_pred.shape)  # Should be [batch_size]
        #print("Length of pred_vec:", len(pred_vec))  # Should match batch_size
        #print("Length of y:", len(original_pred))
        ...
        cor_index = torch.where(sub_pred == original_pred)[0]
       
        err_index = torch.where(sub_pred != original_pred)[0]
        ...
        #print("Shape of SVscore[cor_index]:", SVscore[cor_index].shape)
        #print("Shape of tmp:", tmp.shape)
        #print("Shape of bvsb[cor_index]:", bvsb[cor_index].shape)
        ...
        SVscore[err_index] = (lsh[err_index] - bvsb[err_index])
    ...
    def get_overal_uncertainty(self,uncertainty_score_list, snapshot, score_type=0):
    ...
    def train_sub_model(self, lr, epoch):
        input_length = 768  # Only [CLS] token is used, so hidden size remains 768
        dropout_rate = 0.1   # Dropout probability before classification
        ...
        # Define submodels with Dropout before Linear layer
        self.sub_layers = [
        nn.Sequential(
            nn.Dropout(dropout_rate),
            nn.Linear(input_length, 4)  # multi class classification
        ).to(self.device) for _ in layer_ids
        ]
        ...
        for epoch in range(epoch):
            for layer_features, labels in get_hiddenstate_training(self.model, layer_ids, self.train_loader, self.device, f"Training Epoch {epoch + 1}"):
                for i in range(len(layer_ids)):
                    loss_vals[i] = fit(self.sub_layers[i], layer_features[i], labels, optimizers[i], my_loss, self.device)
            _, pred_y, y = self.predict_with_layers_training(self.dev_loader)
    ...
    def get_uncertainty(self, pred_y, y):
        svscore_list = []
        for sub_pred in pred_y:
            svscore = self.cal_newdissector(sub_pred, y)
    ...
    def predict_with_layers_training(self, dataloader):
        logits = [[] for _ in self.layer_ids]
        preds = [[] for _ in self.layer_ids]
        labels = []

        for layer_features, outputs in get_hiddenstate_training(self.model, self.layer_ids, dataloader, self.device, "Predicting with Layers"):
    ...
    def predict_with_layers(self, dataloader):
        ...
        for layer_features, outputs in get_hiddenstate_inference(self.model, self.layer_ids, dataloader, self.device, "Predicting with Layers"):
            for idx, features in enumerate(layer_features):
                self.sub_layers[idx].eval()
                logit, pred = layer_predict(self.sub_layers[idx], features, self.device)
                logits[idx].append(logit)
    ...
    def _uncertainty_calculate(self, data_loader):
        ...
        logits, preds, y = self.predict_with_layers_training(data_loader)
        
        #lcr_vals = self.get_label_change_rate(preds,y)
        uncertainty_score_list = self.get_uncertainty(logits, y)
        #uncertainty_score_list = self.get_uncertainty_bvsb(logits)
        overal_uncertainty = self.get_overal_uncertainty(uncertainty_score_list, self.layer_ids, 0).detach().cpu()
...
def layer_predict(model, x, device):
    with torch.no_grad():
        ...
        pred_y = F.softmax(logits, dim=1).argmax(dim=1) 
...
def get_hiddenstate_training(model, sub_ids, dataloader, device, message):
    for batch in tqdm(dataloader, desc=message):
        ...
        with torch.no_grad():
            outputs, hidden_state = model.forward_with_hidden_states(inputs)  

            for idx, layer_id in enumerate(sub_ids):
                hidden = hidden_state[layer_id]  # Shape: [batch_size, seq_length, hidden_dim]
                cls_token_representation = hidden[:, 0, :]  # Extract only [CLS] token (shape: [batch_size, 768])
                layer_features[idx] = cls_token_representation.detach().cpu()

        yield layer_features, label.detach().cpu()


def get_hiddenstate_inference(model, sub_ids, dataloader, device, message):
    for batch in tqdm(dataloader, desc=message):
        layer_features = [[] for _ in sub_ids]
        inputs = batch[0].to(device)

        with torch.no_grad():
            outputs, hidden_state = model.forward_with_hidden_states(inputs)  # Get processed sigmoid outputs

            # Convert outputs to binary predictions (0 if <0.5, otherwise 1)
            #predictions = (outputs >= 0.5).long().cpu()  # No need for sigmoid again
        ...
        yield layer_features, predictions 
...
if __name__ == "__main__":
    ...
    test_dataloader = DataLoader(test_dataset, sampler=test_sampler, batch_size=4,num_workers=4,pin_memory=True)
